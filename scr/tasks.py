...
def process_load_dds_extra_tables():
    ...
    for table in extra_tables:
        ...
        if 'год_сертификата' in df.columns:
            ...
            df = df[(df['год_сертификата'].astype(int) >= 1980) & (df['год_сертификата'].astype(int) <= datetime.now().year) & (df['год_сертификата'] != '')]
            df_broken = df[(df['год_сертификата'].astype(int) < 1980) | (df['год_сертификата'].astype(int) > datetime.now().year) | (df['год_сертификата'] == '')]
...
def process_load_dds_base_tables():
    ...
    for table in base_tables.keys():
        ...
        truncate_clear = f"TRUNCATE TABLE andronov_dds.{table} CASCADE;"
        target_hook.run(truncate_clear)
...
def load_data_dm():
    try:
        ...
        raise AirflowException(f"Failed to load dm schema: {e}")